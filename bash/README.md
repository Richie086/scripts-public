...
- `-h`, `--help`, `help`: show usage information and exit
- `-n`, `--dry-run`: show planned actions only, no changes
- `-r`, `--run`: execute full interactive installation

Behavior:

- If no flag is provided, help is shown and the script exits `0`.
- `--run` and `--dry-run` are mutually exclusive (exit `1`).
- Unknown flags print an error and exit `1`.

## Tests

Flag/help coverage (no live install):

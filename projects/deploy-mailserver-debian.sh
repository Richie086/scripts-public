...
#   --rollback                   Uninstall all mail server packages and clean configuration
...
run_rollback() {
  ...
  # Try to detect domain and hostname from configuration before we purge packages
  if [[ -z "$MAIL_HOSTNAME" ]]; then
    if [[ -f /etc/mailname ]]; then
      MAIL_HOSTNAME=$(cat /etc/mailname | tr -d '[:space:]')
    elif [[ -f /etc/postfix/main.cf ]]; then
      MAIL_HOSTNAME=$(grep -Po "(?<=^myhostname = ).*" /etc/postfix/main.cf | tr -d '[:space:]' || true)
    fi
  fi

  if [[ -z "$MAIL_DOMAIN" ]]; then
    if [[ -f /etc/postfix/main.cf ]]; then
      MAIL_DOMAIN=$(grep -Po "(?<=^mydomain = ).*" /etc/postfix/main.cf | tr -d '[:space:]' || true)
    fi
  fi

  log "Detected Domain:   ${MAIL_DOMAIN:-[not detected]}"
  log "Detected Hostname: ${MAIL_HOSTNAME:-[not detected]}"

  log "Stopping and disabling services..."
  for svc in postfix dovecot opendkim; do
    if systemctl list-units --type=service --all | grep -Fq "${svc}.service"; then
      systemctl stop "$svc" || true
      systemctl disable "$svc" || true
    fi
  done

  if [[ -n "$MAIL_HOSTNAME" ]] && command -v certbot &>/dev/null; then
    log "Deleting Let's Encrypt certificate for $MAIL_HOSTNAME..."
    certbot delete --cert-name "$MAIL_HOSTNAME" --non-interactive || true
  fi

  log "Purging packages..."
  export DEBIAN_FRONTEND=noninteractive
  apt-get purge -y -qq postfix postfix-pcre mailutils libsasl2-modules dovecot-core dovecot-imapd opendkim opendkim-tools certbot || true
  
  log "Removing unused dependencies..."
  apt-get autoremove -y --purge -qq || true
  apt-get clean

  log "Cleaning configuration files and directories..."
  rm -rf /etc/postfix
  rm -rf /etc/dovecot
  rm -rf /etc/opendkim
  rm -f /etc/opendkim.conf
  rm -f /etc/mailname
  rm -f /etc/letsencrypt/renewal-hooks/deploy/mailserver-reload.sh
  if [[ -n "$MAIL_HOSTNAME" ]]; then
    rm -rf "/etc/letsencrypt/live/$MAIL_HOSTNAME" "/etc/letsencrypt/archive/$MAIL_HOSTNAME" "/etc/letsencrypt/renewal/$MAIL_HOSTNAME.conf"
  fi

  if [[ -n "$MAIL_HOSTNAME" ]]; then
    log "Removing entry for $MAIL_HOSTNAME from /etc/hosts..."
    sed -i "/127.0.1.1\s\+$MAIL_HOSTNAME/d" /etc/hosts
  fi
  ...
  if command -v ufw &>/dev/null; then
    ...
    ufw delete allow 465/tcp || true
    ufw delete allow 143/tcp || true
    ufw delete allow 993/tcp || true
    ufw delete allow 80/tcp || true
  fi

  log "Rollback complete. System is cleaned."
  
  local current_hostname
  current_hostname=$(hostnamectl hostname 2>/dev/null || hostname || true)
  if [[ "$current_hostname" == "$MAIL_HOSTNAME" ]]; then
    warn "System hostname is still set to '$MAIL_HOSTNAME'."
    warn "You may want to restore it manually via: hostnamectl set-hostname <original-hostname>"
  fi
}

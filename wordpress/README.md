# wordpress

<!-- AUTO-GENERATED MERMAID START -->
<!-- This Mermaid diagram is auto-generated. Do not edit directly. -->

## Directory structure

<details>
<summary>Show directory tree diagram for `wordpress`</summary>

```mermaid
graph TD
	root["wordpress"]:::root --> n1("mermaid-examples"):::folder
	root["wordpress"]:::root --> n2["extremely-sarcastic-mcp-docker.md"]:::file-md
	root["wordpress"]:::root --> n3["jira-csv-import-atlassian-mcp.md"]:::file-md
	root["wordpress"]:::root --> n4["ai-web-app-development-process.md"]:::file-md
	root["wordpress"]:::root --> n5["README.md"]:::file-md
	root["wordpress"]:::root --> n6["ultimate-guide-to-bitwarden-securing-your-digital-life-across-every-device.md"]:::file-md
	root["wordpress"]:::root --> n7["linux-distros-package-managers-part-2.md"]:::file-md
	root["wordpress"]:::root --> n8["gmail-labeling-blog.md"]:::file-md
	root["wordpress"]:::root --> n9["how-to-fix-duplicate-widgets-in-terminal-wordpress-theme.md"]:::file-md
	root["wordpress"]:::root --> n10["aws-ec2-antigravity-blog.md"]:::file-md
	root["wordpress"]:::root --> n11["how_to_use_antigravity.md"]:::file-md
	root["wordpress"]:::root --> n12["automated-aws-free-tier-nginx-ssl-deployment.md"]:::file-md
	root["wordpress"]:::root --> n13["ai-automation.md"]:::file-md
	root["wordpress"]:::root --> n14["test_post.md"]:::file-md
	root["wordpress"]:::root --> n15["install-terraform-ubuntu.md"]:::file-md
	root["wordpress"]:::root --> n16["automating-wordpress-antigravity.md"]:::file-md
	root["wordpress"]:::root --> n17["about.md"]:::file-md
	root["wordpress"]:::root --> n18["how-to-update-rust-desk-pro-self-hosted-docker.md"]:::file-md
	root["wordpress"]:::root --> n19["mermaid-markup-language-guide.md"]:::file-md
	root["wordpress"]:::root --> n20["deploy-mailserver-debian-blog.md"]:::file-md
	root["wordpress"]:::root --> n21["alias-suggester-blog.md"]:::file-md
	root["wordpress"]:::root --> n22["openssl-bash-wrapper.md"]:::file-md
	root["wordpress"]:::root --> n23["technical-writing.md"]:::file-md
	root["wordpress"]:::root --> n24["directory_stats.md"]:::file-md
	root["wordpress"]:::root --> n25["ai-api-keys-obsidian-bitwarden.md"]:::file-md
	root["wordpress"]:::root --> n26["blog_post_wordpress.md"]:::file-md
	root["wordpress"]:::root --> n27["reduce-cursor-credits-blog.md"]:::file-md
	root["wordpress"]:::root --> n28["conquering-docker-permissions-and-antigravity-ui-crashes.md"]:::file-md
	root["wordpress"]:::root --> n29["scripts_catalog.md"]:::file-md
	root["wordpress"]:::root --> n30["antigravity_blog_post.md"]:::file-md
	root["wordpress"]:::root --> n31["installing-vscode.md"]:::file-md
	root["wordpress"]:::root --> n32["installing-vscode.html"]:::file-html
	root["wordpress"]:::root --> n33["linux-history-part-1.md"]:::file-md
	n1 --> n1_5["README.md<br>sequence.md<br>flowchart.md<br>state.md<br>gantt.md"]:::file-md
classDef root fill:#2563eb,stroke:#1e40af,stroke-width:2px,color:#ffffff;
classDef folder fill:#fbbf24,stroke:#d97706,stroke-width:1px,color:#451a03;
classDef file-html fill:#ffedd5,stroke:#ea580c,stroke-width:1px,color:#7c2d12;
classDef file-md fill:#dbeafe,stroke:#3b82f6,stroke-width:1px,color:#1e3a8a;
```

</details>

<!-- AUTO-GENERATED MERMAID END -->

Auto-generated directory structure for this folder.
